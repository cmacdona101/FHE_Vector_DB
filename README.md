...
This is done via fully homomorphic encryption, which allows some mathematical operations to operate on encrypted data directly without first needing the decryption -> operation -> re-encryption stages.  The final result is passed back to the user as an encrypted list of top k results (as well as several low-ranking words for control comparisons in this proof of concept test), which are decrypted with the user's private key. 
